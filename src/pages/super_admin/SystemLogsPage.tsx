...
import { FileText, Clock, User, Filter, Loader2, AlertCircle, RefreshCw, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  const loadLogs = useCallback(async () => {
    try {
      ...
      const enrichedLogs = await Promise.all(
        (data || []).map(async (log: JournalActivite) => {
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.id}>
                      ...
                      <td className={styles['sa-system-logs__ip']}>{log.adresse_ip || '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
