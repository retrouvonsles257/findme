...
} from '../../../features/authority/services';
import {
  UserCheck,
  Search,
  Filter,
...
export const IdentityVerificationPage: React.FC<IdentityVerificationPageProps> = ({ noLayout = false, organisationId = null }) => {
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        action === 'approuve'
          ? t('moderation.identityVerifiedSuccess')
          : action === 'refuse'
            ? t('moderation.requestRejected')
            : t('moderation.complementRequested')
      );
      ...
      setErrorMessage(err.message || t('moderation.errorProcessing'));
      setTimeout(() => setErrorMessage(''), 5000);
    } finally {
  ...
  const getStatusInfo = (status: string) => {
    const info: Record<string, { label: string; color: string }> = {
      en_attente: { label: t('moderation.identityStatusPending'), color: '#f59e0b' },
      en_cours: { label: t('moderation.identityStatusInProgress'), color: '#38bdf8' },
      approuve: { label: t('moderation.identityStatusApproved'), color: '#0284c7' },
      rejete: { label: t('moderation.identityStatusRejected'), color: '#ef4444' },
      complement_demande: { label: t('moderation.identityStatusComplement'), color: '#8b5cf6' },
    };
        ...
        <section className={styles['identity-verification__header']}>
          <div className={styles['identity-verification__header-content']}>
            <h1 className={styles['identity-verification__title']}>
              <UserCheck size={28} />
              {t('moderation.identityTitle')}
            </h1>
            <p className={styles['identity-verification__subtitle']}>
              {t('moderation.identitySubtitle')}
            </p>
          </div>
          ...
          <div className={styles['identity-verification__toolbar']}>
            <div className={styles['identity-verification__search-wrapper']}>
              ...
              <input
                type="text"
                placeholder={t('moderation.searchPlaceholderIdentity')}
            ...
            <div className={styles['identity-verification__filters']}>
              <div className={styles['identity-verification__filter-group']}>
                ...
                >
                  ...
                  <option value="en_attente">{t('moderation.identityStatusPending')}</option>
                  <option value="en_cours">{t('moderation.identityStatusInProgress')}</option>
                  <option value="approuve">{t('moderation.identityStatusApproved')}</option>
                  <option value="rejete">{t('moderation.identityStatusRejected')}</option>
                  <option value="complement_demande">{t('moderation.identityStatusComplement')}</option>
                </select>
              </div>

              <div className={styles['identity-verification__filter-group']}>
                <label>{t('moderation.identityDocumentTypeLabel')}</label>
                <select
                  value={filters.documentType}
                  onChange={(e) => setFilters(prev => ({ ...prev, documentType: e.target.value as any }))}
                >
                  <option value="all">{t('moderation.iaTypeAll')}</option>
                  <option value="cni">{t('moderation.typeCni')}</option>
                  <option value="passeport">{t('moderation.passport')}</option>
              ...
              <div className={styles['identity-verification__filter-group']}>
                ...
                >
                  <option value="all">{t('moderation.identityPeriodAll')}</option>
        ...
        <div className={styles['identity-verification__stats']}>
          <div className={styles['identity-verification__stat']}>
            ...
            <div>
              ...
              <span className={styles['identity-verification__stat-label']}>{t('moderation.identityTotalLabel')}</span>
            </div>
          </div>
          <div className={styles['identity-verification__stat']}>
            <Clock size={24} />
            <div>
              ...
              <span className={styles['identity-verification__stat-label']}>{t('moderation.identityStatusPending')}</span>
            </div>
          </div>
          <div className={styles['identity-verification__stat']}>
            <CheckCircle size={24} />
            <div>
              ...
              <span className={styles['identity-verification__stat-label']}>{t('moderation.identityVerifiedLabel')}</span>
            </div>
          </div>
          <div className={styles['identity-verification__stat']}>
            <XCircle size={24} />
            <div>
              ...
              <span className={styles['identity-verification__stat-label']}>{t('moderation.identityRejectedLabel')}</span>
            </div>
          </div>
        </div>
            ...
            <div className={styles['identity-verification__grid']}>
              {requests.length > 0 ? (
                    ...
                    >
                      ...
                      <div className={styles['identity-verification__card-body']}>
                        ...
                        <p className={styles['identity-verification__date']}>
                          <Calendar size={14} />
                          {t('moderation.registeredOn')} {new Date(user?.created_at || '').toLocaleDateString('fr-FR')}
                        </p>
                      </div>
                      ...
                      <div className={styles['identity-verification__card-footer']}>
                        <div className={styles['identity-verification__doc-type']}>
                          <CreditCard size={16} />
                          <span>
                            {request.type_document === 'cni' ? t('moderation.typeCni') : 
                             request.type_document === 'passeport' ? t('moderation.passport') : t('common.other')}
                          </span>
                        </div>
                        <button className={styles['identity-verification__view-btn']}>
                          <Eye size={16} />
                          {t('moderation.examine')}
                        </button>
                      </div>
                    </div>
                  );
                ...
                <div className={styles['identity-verification__empty']}>
                  <UserCheck size={48} />
                  <p>{t('moderation.noVerificationRequests')}</p>
                </div>
              )}
            </div>
            ...
            >
              ...
              <div className={styles['identity-verification__modal-body']}>
                ...
                <div className={styles['identity-verification__info-section']}>
                  <h4>{t('moderation.accountInfo')}</h4>
                  <div className={styles['identity-verification__info-grid']}>
                    ...
                    {selectedRequest.utilisateur?.date_naissance && (
                      <div className={styles['identity-verification__info-row']}>
                        <label>{t('moderation.dateOfBirth')}</label>
                ...
                <div className={styles['identity-verification__doc-section']}>
                  <h4>
                    <FileText size={18} />
                    {t('moderation.identityDocumentTitle')}
                  </h4>
                  <div className={styles['identity-verification__doc-type-badge']}>
                    <CreditCard size={16} />
                    {selectedRequest.type_document === 'cni' ? t('moderation.nationalIdCard') : 
                     selectedRequest.type_document === 'passeport' ? t('moderation.passport') : t('moderation.otherDocument')}
                  </div>
                  ...
                  {selectedRequest.url_document ? (
                    <div className={styles['identity-verification__doc-preview']}>
                      <img 
                        ...
                        alt={t('moderation.identityDocumentTitle')}
                      ...
                      >
                        <Download size={16} />
                        {t('moderation.viewFullScreen')}
                      </a>
                    </div>
                  ) : (
                    <div className={styles['identity-verification__no-doc']}>
                      <AlertTriangle size={24} />
                      <p>{t('moderation.noDocumentSubmitted')}</p>
                    </div>
                  )}
                </div>
                ...
                {selectedRequest.statut === 'en_attente' && (
                  <div className={styles['identity-verification__decision-section']}>
                    <h4>
                      <Shield size={18} />
                      {t('moderation.verificationDecision')}
                    </h4>
                    ...
                    <div className={styles['identity-verification__decision-buttons']}>
                      ...
                      >
                        ...
                        {t('moderation.requestComplement')}
                      </button>
                    </div>

                    {decisionData.decision === 'rejete' && (
                      <div className={styles['identity-verification__form-group']}>
                        <label>{t('moderation.rejectReasonLabel')}</label>
                        <select
                        ...
                        >
                          ...
                          <option value="document_illisible">{t('moderation.reasonIllegible')}</option>
                          <option value="document_expire">{t('moderation.reasonExpired')}</option>
                          <option value="document_falsifie">{t('moderation.reasonFake')}</option>
                          <option value="informations_incoherentes">{t('moderation.reasonInconsistent')}</option>
                          <option value="photo_non_conforme">{t('moderation.reasonPhotoNonCompliant')}</option>
                          <option value="autre">{t('common.other')}</option>
                        </select>
                      </div>
                    )}
                    ...
                    <div className={styles['identity-verification__form-group']}>
                      ...
                      <textarea
                        ...
                        placeholder={t('moderation.notesPlaceholderInternal')}
                        rows={3}
                      />
                    </div>
                  ...
                  <div className={styles['identity-verification__already-processed']}>
                    <p>
                      ...
                      {selectedRequest.statut === 'approuve'
                        ? t('moderation.alreadyProcessedApproved')
                        : selectedRequest.statut === 'rejete'
                          ? t('moderation.alreadyProcessedRejected')
                          : t('moderation.alreadyProcessedComplement')}
  ...
  return (
    <ModerationLayout title={t('moderation.identityPageTitle')} activeNav="identity">
      {content}
    </ModerationLayout>
  );
};
